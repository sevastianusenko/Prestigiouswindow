    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-5 sm:px-8 py-20">
        ...
        <ul className="mt-10 grid sm:grid-cols-2 gap-x-10 gap-y-4">
          {signs.map((s) => (
            <li key={s} className="flex gap-3 text-graphite/75 border-t border-mist pt-4">
      ...
      <section className="mx-auto max-w-7xl px-5 sm:px-8 py-20">
        ...
        <div className="mt-10 grid sm:grid-cols-2 gap-px bg-mist">
          {options.map((o) => (
            <div key={o.name} className="bg-quartz p-6 sm:p-8">
              <h3 className="font-display text-xl font-semibold">{o.name}</h3>
              <p className="mt-2 text-sm text-graphite/70">{o.detail}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="bg-graphite text-quartz py-20">
        <div className="mx-auto max-w-7xl px-5 sm:px-8 grid lg:grid-cols-2 gap-12 items-center">
          <QuoteForm />
          <div>
            <Eyebrow>What it costs</Eyebrow>
            <SectionHeading as="h2" className="text-2xl sm:text-3xl text-quartz">
              We don&apos;t post prices — here&apos;s why
            </SectionHeading>
            <p className="mt-4 text-quartz/70 max-w-md">
